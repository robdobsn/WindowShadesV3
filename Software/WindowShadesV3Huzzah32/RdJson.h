...
public:
                                 ...
                                 int maxTokens = 10000)
    {
        ...
        int tokenCountRslt = JSMNR_parse(&parser, jsonStr, strlen(jsonStr),
                                         NULL, maxTokens);
        if (tokenCountRslt < 0) {
            Log.trace(F("RdJson: parseJson result %d maxTokens %d jsonLen %d"CR), tokenCountRslt, maxTokens, strlen(jsonStr));
        ...
        if (tokenCountRslt < 0) {
            Log.notice(F("parseJson result: %d"CR), tokenCountRslt);
            Log.trace(F("jsonStr %s numTok %d maxTok %d"CR), jsonStr, numTokens, maxTokens);
            delete[] pTokens;
            return NULL;
        }
...
private:
                                   ...
                                   int& startTokenIdx, int& endTokenIdx)
    {
        ...
        if (keyIdx < 0) {
            //Log.notice(F("getTokenByDataPath not found %s"CR), dataPath);
            return false;
        }
    ...
    {
        // Log.trace(F("findObjectEnd idx %d, count %d, start %s"CR), curTokenIdx, count,
        ...
        for (int objIdx = 0; objIdx < count; objIdx++) {
            ...
            if (pTok->type == JSMNR_PRIMITIVE) {
                // Log.trace(F("findObjectEnd PRIMITIVE"CR));
                tokIdx += 1;
            }
            else if (pTok->type == JSMNR_STRING) {
                // Log.trace(F("findObjectEnd STRING"CR));
                if (atObjectKey) {
        ...
        // Log.trace(F("findObjectEnd returning %d, start %s, end %s"CR), tokIdx,
                             ...
                             jsmnrtype_t keyType = JSMNR_UNDEFINED)
    {
        ...
        while (pDataPathPos <= dataPath + strlen(dataPath)) {
            ...
            // Log.trace(F("SlashPos %ld, %ld, srchKey <%s>"CR), slashPos, slashPos-pDataPathPos, srchKey);
            ...
            // Log.trace(F("findKeyInJson srchKey %s arrayIdx %ld"CR), srchKey, reqdArrayIdx);
            ...
            for (int tokIdx = curTokenIdx; tokIdx <= maxTokenIdx;) {
                ...
                // Log.trace(F("Token type %d"CR), pTok->type);
...
//                            Log.trace(F("TokIdxArray inIdx %d, reqdArrayIdx %d, outTokIdx %d"CR), tokIdx, reqdArrayIdx, newTokIdx);
                            tokIdx = newTokIdx;
                        }
                        else {
                    ...
                    if (atNodeLevel) {
                        // Log.trace(F("findObjectEnd we have got it %d"CR), tokIdx);
                        if ((keyTypeToFind == JSMNR_UNDEFINED) || (tokens[tokIdx].type == keyTypeToFind)) {
                            ...
                            //Log.trace(F("TokIdxDiff max %d, test %d, diff %d"CR), endTokenIdx, testTokenIdx, testTokenIdx-endTokenIdx);
                            return tokIdx;
                        }
                        return -1;
                    }
                        ...
                        // Check for an object
                        // Log.trace(F("findObjectEnd inside"CR));
                        if (tokens[tokIdx].type == JSMNR_OBJECT) {
                            ...
                            //int testTokenIdx = findObjectEnd(jsonOriginal, tokens, numTokens, tokIdx+1, 1);
                            //Log.trace(F("TokIdxDiff2 max %d, test %d, diff %d"CR), maxTokenIdx, testTokenIdx, testTokenIdx- maxTokenIdx);
...
public:
                               ...
                               bool skipJSONWhitespace = false)
    {
        ...
        //        Log.trace(F("safeStringDup <%s> %d %d %d %d %d %d %d <%s>"CR), pSrc, maxx, toAlloc, srcStrlen, stringLen, insideDoubleQuotes, insideSingleQuotes, skipJSONWhitespace, pDest);
        return pDest;
    }
