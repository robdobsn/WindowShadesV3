...
public:
    ...
    const RdWebServerResourceDescr* handleReceivedHttp(bool& handledOk, RdWebServer *pWebServer);
private:
    ...
    // Resource to send
    const RdWebServerResourceDescr* _pResourceToSend;
...
public:
    ...
    void addStaticResources(const RdWebServerResourceDescr *pResources, int numResources);
    ...
    const RdWebServerResourceDescr* getResource(int resIdx)
    {
...
private:
    ...
    const RdWebServerResourceDescr *_pWebServerResources;
...
public:
    ...
    {
        return ((unsigned long)time(NULL) < _webServerActiveLastUnixTime + nSeconds);
    }
};
