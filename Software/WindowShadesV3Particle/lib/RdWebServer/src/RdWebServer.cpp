...
void RdWebClient::setState(WebClientState newState)
{
    ...
    {
        Log.trace("%09lu WebClient %d State: %s entryMs %ld", micros(), _clientIdx, connStateStr(), _webClientStateEntryMs);
    }
}
...
void RdWebClient::handleTCPReadData(int numToRead)
{
    ...
    if (!_httpHeaderComplete)
    {
        ...
        if (_httpHeaderComplete)
        {
            ...
            _curHttpPayloadRxPos = 0;
            Log.trace("%09lu WebClient Payload length %d", micros(), payloadLen);
...
void RdWebClient::service(RdWebServer *pWebServer)
{
    ...
    case WEB_CLIENT_NONE:
        ...
        if (_TCPClient)
        {
            ...
            String    ipStr = ip;
            Log.trace("%09lu WebClient IP %s", micros(), ipStr.c_str());
        }
        break;
    ...
    case WEB_CLIENT_ACCEPTED:
       {
           ...
           {
               Log.trace("%09lu WebClient disconnected", micros());
               _TCPClient.stop();
               cleanupTCPRxResources();
           ...
           {
               Log.trace("%09lu WebClient no-data timeout", micros());
               _TCPClient.stop();
               cleanupTCPRxResources();
               setState(WEB_CLIENT_NONE);
           }
           ...
           {
               Log.trace("%09lu WebClient received %d", micros(), _httpReqStr.length());
               ...
               if (!handledOk)
               {
                   Log.trace("%09lu WebClient couldn't handle request", micros());
               }
           }
           else
           {
    ...
    case WEB_CLIENT_SEND_RESOURCE:
       {
           if (!_pResourceToSend)
           {
               ...
               _TCPClient.stop();
               setState(WEB_CLIENT_NONE);
               Log.trace("%09lu WebClient resp complete", micros());
               break;
           }
           ...
           if (_resourceSendIdx >= _pResourceToSend->_dataLen)
           {
               ...
               _TCPClient.stop();
               setState(WEB_CLIENT_NONE);
               Log.trace("%09lu WebClient Sent %s, %d bytes total, %d blocks", micros(),
...
RdWebServerResourceDescr *RdWebClient::handleReceivedHttp(bool& handledOk, RdWebServer *pWebServer)
{
    ...
    if (extractEndpointArgs(pHttpReq + 3, endpointStr, argStr))
    {
        ...
        Log.trace("%09lu WebClient handleHTTP EndPtStr %s ArgStr %s", micros(),
        ...
        if (pEndpoint)
        {
            Log.trace("%09lu WebClient FoundEndpoint <%s> Type %d", micros(),
            ...
            if (pEndpoint->_endpointType == RestAPIEndpointDef::ENDPOINT_CALLBACK)
            {
                ...
                Log.trace("%09lu WebClient api response len %d", micros(), retStr.length());
                ...
                Log.trace("%09lu WebClient http response len %d", micros(), _httpRespStr.length());
                ...
                Log.trace("%09lu WebClient write to tcp clientIdx %d len %d", micros(), _clientIdx, _httpRespStr.length());
                // See comment above
                delay(20);
                _TCPClient.flush();
                handledOk = true;
        ...
        if (!handledOk)
        {
            for (int wsResIdx = 0; wsResIdx < pWebServer->getNumResources(); wsResIdx++)
            {
                    ...
                    ((strlen(endpointStr) == 0) && (strcasecmp(pRes->_pResId, "index.html") == 0)))
                {
                    if (pRes->_pData != NULL)
                    {
                        Log.trace("%09lu WebClient sending resource %s, %d bytes, %s",
        ...
        if (!handledOk)
        {
            Log.trace("%09lu WebClient Endpoint %s not found or invalid", micros(), endpointStr.c_str());
        }
    }
    else
    {
        Log.trace("%09lu WebClient Cannot find command or args", micros());
    }

    // Handle situations where the command wasn't handled ok
    if (!handledOk)
    {
        Log.trace("%09lu WebClient Returning 404 Not found", micros());
